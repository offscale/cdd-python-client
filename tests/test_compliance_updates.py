...
def test_emit_function_requestBody():
    from openapi_client.models import OpenAPI, Operation, RequestBody
    from openapi_client.functions.emit import emit_function
    op = Operation(
...
def test_emit_tests():
    ...
    op = Operation(
        operationId="post_things",
        requestBody=RequestBody(content={"application/json": {"schema": Schema(type="object", properties={"name": Schema(type="str"), "photoUrls": Schema(type="list[str]"), "other": Schema(type="str")})}}),
        parameters=[
    ...
    spec = OpenAPI(
        ...
        paths={"/things": PathItem(post=op), "/inventory": PathItem(get=Operation(operationId="get_inv"))}
    )
...
def test_get_dummy_value_for_schema():
    ...
    assert "test_string" in str(get_dummy_value_for_schema(Schema(type="array", items=Schema(type="string"))))
    assert "1" in str(get_dummy_value_for_schema(Schema(type="array", items=Schema(type="integer"))))
