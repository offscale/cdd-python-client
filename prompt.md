...
4. Verify your changes by ensuring `cd .. && ONLY_TEST=cdd-python-all ./local-test.sh only-test` passes successfully (including the coverage validation and chaos saboteur audits).
...
The Python SDK fails the Coverage Validation stage due to untested endpoints (e.g., `PUT /v2/pet`, `GET /v2/store/inventory`). Add the missing endpoint assertions to the generated test suite.
